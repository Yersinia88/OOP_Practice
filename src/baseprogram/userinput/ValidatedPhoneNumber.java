...
public class ValidatedPhoneNumber {
    ...
    public boolean validationOfThePhoneNumber(String phoneNumber) {
        ...
        String[] dialingCodes = {String.valueOf(MobileServiceTypes.DECENS.getDialingCode()), String.valueOf(MobileServiceTypes.FECSEGTETO.getDialingCode()), String.valueOf(MobileServiceTypes.LUKRACIO.getDialingCode())};
        ...
        if (phoneNumber.contains("+") && !phoneNumber.startsWith("+")) { //Tartalmaz-e rossz helyen +-t
            System.out.println("A megadott telefonszám nem értelmezhető!");
            return false;
        }

        for (int i = 0; i < phoneNumber.length(); i++) { //Megnéztem, hogy tartalmaz-e nem valid karaktert
